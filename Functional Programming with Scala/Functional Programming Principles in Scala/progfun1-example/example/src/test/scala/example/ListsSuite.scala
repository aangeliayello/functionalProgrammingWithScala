...
class ListsSuite extends munit.FunSuite:
  ...
  test("max should return the largest element in the list") {
    val testCases = List(
      (List(1, 2, 3, 4, 5), 5),
      (List(10, 20, 30), 30),
      (List(0, 0, 0, 0), 0)
    ...
    testCases.foreach { case (input, expected) =>
      val result = Lists.max(input)
      assertEquals(result, expected)
    }
  }

  test("max should throw NoSuchElementException for an empty list") {
    interceptMessage[NoSuchElementException]("List provided is empty.") {
      val input = List()
      Lists.max(input)
    }
  }
